...
async function loadTercerosDZ() {
  // terceros_diaz es la fuente única de la verdad para Díaz.
  // Contiene: Cliente-Corporativo, Cliente-Individual, Proveedor, Acreedor,
  // Socio, Empresa relacionada, Empleado. clientes_diaz es una vista sobre
  // esta tabla filtrada por tipo LIKE 'Cliente%'.
  const {data, error} = await db.from('terceros_diaz').select('*').order('nombre');
  TERCEROS_DZ = data || [];
  DZ_TERCEROS_FALLBACK = false;
  if(error) console.error('Error cargando terceros_diaz:', error.message);
  renderTercerosDZ();
}

// Normaliza cualquier variante de tipo Cliente a un token unificado.
// Se usa para que el filtro "cliente" del módulo Terceros capture
// Cliente, Cliente-Corporativo y Cliente-Individual sin duplicar código.
function normalizarTipoTercero(tipo){
  var t = (tipo||'').toString().trim();
  if(!t) return '';
  if(t.toLowerCase().indexOf('cliente') === 0) return 'cliente';
...
function renderTercerosDZ() {
  ...
  var rows = TERCEROS_DZ.filter(function(r) {
    if(TERCEROS_DZ_FILTER !== 'todos'){
      if(normalizarTipoTercero(r.tipo) !== TERCEROS_DZ_FILTER.toLowerCase()) return false;
    }
    if(q && !(r.nombre||'').toLowerCase().includes(q) && !(r.email||'').toLowerCase().includes(q)) return false;
    return true;
  });
  ...
  var cols = {
    'Cliente':'#00A98D',
    'Cliente-Corporativo':'#00A98D',
    'Cliente-Individual':'#5FBFA8',
    'Proveedor':'#d4870a',
    'Acreedor':'#e06200',
    'Empleado':'#5B8DB8',
    'Empresa relacionada':'#185FA5',
    'Socio':'#7C3AED'
  };
  if (!rows.length) {
...
function openNuevoTercero(empresa) {
  ...
  var tipos = esDiaz
    ? ['Cliente-Corporativo','Cliente-Individual','Proveedor','Acreedor','Socio','Empresa relacionada','Empleado']
    : ['Inversionista','Proveedor','Acreedor','Socio','Empresa relacionada','Empleado'];
...
async function openNuevaFactura() {
  var {data: clientes, error: errClientes} = await db.from('clientes_diaz').select('nombre').order('nombre');
  if(errClientes){ console.error('Error cargando clientes_diaz:', errClientes.message); }
  var optsCliente = (clientes||[]).map(function(c){ return '<option value="'+c.nombre+'">'+c.nombre+'</option>'; }).join('');
