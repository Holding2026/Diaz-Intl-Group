...
function fcRenderDashboard(empresa, trans, anio, ac, body, resumen, saldos) {
  ...
  <div style="background:var(--sf);border:1px solid var(--br);border-radius:10px;padding:16px 18px;margin-bottom:16px">
    ...
    ${(()=>{
      if(!saldoMensual.length) return '<div style="text-align:center;color:var(--t3);font-size:11px;padding:24px">Sin datos · usa 📤 Importar extracto</div>';
...
async function fcCambiarCat(ev,id){
  ...
  else{ev.target.style.background=nombre?'rgba(46,204,113,0.2)':'';setTimeout(()=>{ev.target.style.background='var(--ad)';},800);}
}
...
async function irsAbrir(empresa){
  ...
  const autos={},detalleOtros={},excluidas={ing:0,eg:0};
  trans.forEach(t=>{
    ...
    if((t.egreso||0)>0){
      ...
      if(l==='e34')detalleOtros[cat]=(detalleOtros[cat]||0)+t.egreso;
    }
  });
  const ajustes={};
  (ajustesRows||[]).forEach(a=>{if(a.valor!=null)ajustes[a.linea]=parseFloat(a.valor);});

  IRS_STATE={empresa,anio,autos,ajustes,saldoFinal,detalleOtros,excluidas};
  irsRender();
}
...
function irsFilaHTML(L,esAdmin){
  ...
  let extra='';
  if(L.c==='e34'&&Object.keys(IRS_STATE.detalleOtros).length){
    extra='<div style="font-size:9px;color:var(--t3);margin-top:2px">'+
      Object.entries(IRS_STATE.detalleOtros).sort((a,b)=>b[1]-a[1]).map(([k,v])=>k+': '+fm(v)).join(' · ')+'</div>';
  }
