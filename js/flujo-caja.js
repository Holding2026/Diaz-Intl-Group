...
async function loadCont(empresa) {
  ...
  if(!filt.cuenta){
    const idsExtra=(FC_CUENTAS[empresa]||[]).filter(c=>c.es_bancaria===false).map(c=>c.id);
    if(idsExtra.length) q=q.not('cuenta_id','in','('+idsExtra.join(',')+')');
  }
  if(anio>0){
...
async function fcImportarInvConfirmar(){
  ...
  const rows=checks.map(chk=>{
    ...
    return {row:r,tx:{empresa_id:'tycoon',cuenta_id:cuentaExtra.id,fecha:r.fecha,concepto:'Pago directo a '+r.inversionista+(r.numero?' ('+r.numero+')':''),egreso:r.monto,ingreso:0,categoria_nombre:cat,origen:'extracontable_inversionista'}};
  });
  let ok=0,fail=0;
  ...
  $('fii-err').style.color='var(--ac)';
  $('fii-err').textContent=`✅ ${ok} pagos importados al Extracontable${fail?` · ${fail} fallaron`:''}`;
