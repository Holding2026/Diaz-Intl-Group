async function loadNomina(empresa) {
  ...
  var rows = data||[];
  await enriquecerConAbonos(rows);
  NOMINA_DATA[empresa] = rows;
  renderNomina(empresa);
}

// ══════════════════════════════════════════════════════════════
// ABONOS PARCIALES DE NÓMINA — saldo pendiente y mora en vivo
// ══════════════════════════════════════════════════════════════
// Trae los abonos de un conjunto de pagos y adjunta a cada fila:
// _abonos, _totalAbonado, _totalAbonadoUSD, _saldo, _saldoUSD, _diasMora
// (mora calculada al momento de renderizar, no depende de guardar el registro)
async function enriquecerConAbonos(rows) {
  if(!rows || !rows.length) return rows;
  var ids = rows.map(function(r){return r.id;}).filter(Boolean);
  var abonosMap = {};
  if(ids.length){
    var {data: abonos} = await db.from('nomina_abonos').select('*').in('nomina_pago_id', ids).order('fecha',{ascending:true});
    (abonos||[]).forEach(function(a){
      if(!abonosMap[a.nomina_pago_id]) abonosMap[a.nomina_pago_id] = [];
      abonosMap[a.nomina_pago_id].push(a);
    });
  }
  var hoy = new Date(); hoy.setHours(12,0,0,0);
  rows.forEach(function(r){
    var abs = abonosMap[r.id]||[];
    r._abonos = abs;
    r._totalAbonado = abs.reduce(function(s,a){return s+(a.valor||0);},0);
    r._totalAbonadoUSD = abs.reduce(function(s,a){return s+(a.valor_usd||0);},0);
    r._saldo = Math.max((r.valor_neto||0) - r._totalAbonado, 0);
    r._saldoUSD = (r.valor_neto_usd!=null) ? Math.max(r.valor_neto_usd - r._totalAbonadoUSD, 0) : null;
    r._diasMora = null;
    if(r.estado !== 'Pagado' && r.fecha_pago){
      var prog = new Date(r.fecha_pago+'T12:00:00');
      r._diasMora = Math.round((hoy-prog)/86400000);
    } else if(r.dias_mora != null) {
      r._diasMora = r.dias_mora;
    }
  });
  return rows;
}
...
function renderNomina(empresa) {
  ...
  var pendRows   = rows.filter(function(r){return r.estado==='Pendiente'||r.estado==='Abono';});
  var saldoPend  = pendRows.reduce(function(a,r){return a+(r._saldo!=null?r._saldo:(r.valor_neto||0));},0);
  ...
  var kpis = [
    ...
    ['Saldo pendiente',pendRows.length+' registro(s) — incluye abonos',fm(saldoPend),saldoPend>0?'var(--or)':'var(--t3)']
  ].map(function(k){
  ...
  if(!body) return;
  var estCol = {Pagado:'#00A98D',Pendiente:'#d4870a',Procesando:'#5B8DB8',Abono:'#b8590a'};
  ...
  body.innerHTML = rows.length ? rows.map(function(r){
    ...
    return '<tr>'
      ...
      +'<td style="font-family:monospace;font-size:11px;font-weight:600;color:#00D5B0">'+(r.valor_neto?fm(r.valor_neto):'—')
        +(esAbono?'<br><span style="font-size:9px;color:#b8590a;font-weight:700">Abonado '+fm(r._totalAbonado||0)+' · saldo '+fm(r._saldo||0)+'</span>':'')+'</td>'
      ...
      +(function(){
        ...
        var mora = '';
        var dm = r._diasMora!=null ? r._diasMora : r.dias_mora;
        if(dm!=null) {
          if(dm>0) mora = '<br><span style="font-size:9px;color:#c0392b;font-weight:600">+'+dm+'d mora</span>';
          else if(dm<0) mora = '<br><span style="font-size:9px;color:#00554B">⚡'+Math.abs(dm)+'d antes</span>';
...
async function editarPagoNomina(id, empresa) {
  ...
  // Abonos previos registrados contra este pago
  var {data: abonosData} = await db.from('nomina_abonos').select('*').eq('nomina_pago_id', id).order('fecha',{ascending:true});
  var abonos = abonosData || [];
  var totalAbonado = abonos.reduce(function(s,a){return s+(a.valor||0);},0);
  var saldoPendiente = Math.max((p.valor_neto||0) - totalAbonado, 0);

  // Mora en vivo: si ya está Pagado usar el histórico; si no, calcular contra hoy
  var moraHTML = '';
  if (esPagado && p.fecha_pago && p.fecha_pago_real) {
  ...
  } else if (!esPagado && p.fecha_pago) {
    var hoy2 = new Date(); hoy2.setHours(12,0,0,0);
    var progAbierto = new Date(p.fecha_pago+'T12:00:00');
    var diasMoraLive = Math.round((hoy2-progAbierto)/86400000);
    if (totalAbonado > 0) {
      var moraTxt = diasMoraLive>0 ? '<strong>+'+diasMoraLive+' días de mora</strong>' : (diasMoraLive<0 ? 'faltan '+Math.abs(diasMoraLive)+' días' : 'vence hoy');
      moraHTML = '<div style="background:rgba(184,89,10,0.08);border:1px solid rgba(184,89,10,0.3);border-radius:7px;padding:10px 12px;font-size:11px;color:#b8590a;margin-bottom:10px">'
        +'💰 Abono parcial recibido: <strong>'+fm(totalAbonado)+'</strong> de '+fm(p.valor_neto||0)+' · Saldo pendiente: <strong>'+fm(saldoPendiente)+'</strong>'
        +(p.fecha_pago?'<br>Fecha límite original: '+fd(p.fecha_pago)+' — '+moraTxt:'')+'</div>';
    } else if (diasMoraLive > 0) {
      moraHTML = '<div style="background:rgba(192,57,43,0.08);border:1px solid rgba(192,57,43,0.25);border-radius:7px;padding:8px 12px;font-size:11px;color:#c0392b;margin-bottom:10px">⚠ Vencido — <strong>'+diasMoraLive+' días de mora</strong> (límite: '+fd(p.fecha_pago)+')</div>';
    }
  }
  // Historial de abonos (si hay más de uno)
  var historialAbonosHTML = '';
  if (abonos.length) {
    historialAbonosHTML = '<div style="background:var(--sf2);border-radius:7px;padding:8px 12px;margin-bottom:10px">'
      +'<div style="font-size:9px;color:var(--t3);font-family:monospace;letter-spacing:1px;text-transform:uppercase;margin-bottom:6px">Abonos registrados ('+abonos.length+')</div>'
      +abonos.map(function(a){
        return '<div style="display:flex;justify-content:space-between;font-size:11px;padding:3px 0;border-bottom:1px dashed var(--br)">'
          +'<span style="color:var(--t2)">'+fd(a.fecha)+(a.cuenta_pago?' · '+a.cuenta_pago:'')+'</span>'
          +'<span style="font-family:monospace;font-weight:600;color:#00A98D">'+fm(a.valor)+'</span></div>';
      }).join('')
      +'<div style="display:flex;justify-content:space-between;font-size:11px;padding-top:5px;font-weight:700">'
        +'<span>Saldo pendiente</span><span style="font-family:monospace;color:'+(saldoPendiente>0?'#c0392b':'#00A98D')+'">'+fm(saldoPendiente)+'</span></div>'
      +'</div>';
  }
  ...
  html += '<div><div style="font-size:9px;color:var(--t3);font-family:monospace;letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">Estado</div>'
    ...
    +'<option '+(p.estado==='Abono'?'selected':'')+'>Abono</option>'
  ...
  // REGISTRO DE ABONO PARCIAL
  if (!esPagado) {
    html += sec('Registrar abono (pago parcial)');
    html += '<div style="background:rgba(184,89,10,0.06);border:1px solid rgba(184,89,10,0.25);border-radius:8px;padding:12px 14px;margin-bottom:10px">';
    html += '<div style="font-size:10px;color:var(--t2);margin-bottom:10px">Usa esto cuando solo llegó una parte del pago. El saldo queda abierto y su mora se calcula automáticamente hasta que se complete.</div>';
    html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:8px">';
    html += '<div><div style="font-size:9px;color:var(--t3);font-family:monospace;letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">Monto abonado ('+(p.moneda||'COP')+')</div>'
      +'<input id="ab-valor" type="number" placeholder="Ej: 500000" style="width:100%;background:var(--sf2);border:1px solid var(--br);border-radius:7px;padding:7px 10px;color:var(--t);font-size:12px;outline:none"></div>';
    html += '<div><div style="font-size:9px;color:var(--t3);font-family:monospace;letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">Fecha del abono</div>'
      +'<input id="ab-fecha" type="date" value="'+new Date().toISOString().split('T')[0]+'" style="width:100%;background:var(--sf2);border:1px solid var(--br);border-radius:7px;padding:7px 10px;color:var(--t);font-size:12px;outline:none"></div>';
    html += '</div>';
    html += '<div style="margin-bottom:8px"><div style="font-size:9px;color:var(--t3);font-family:monospace;letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">Cuenta / método</div>'
      +'<input id="ab-cuenta" type="text" placeholder="Bancolombia, Nequi, Bre-B..." style="width:100%;background:var(--sf2);border:1px solid var(--br);border-radius:7px;padding:7px 10px;color:var(--t);font-size:12px;outline:none"></div>';
    html += '<button onclick="guardarAbonoNomina(\''+id+'\',\''+empresa+'\')" style="width:100%;background:#b8590a;color:#fff;border:none;border-radius:7px;padding:10px;font-size:13px;font-weight:700;cursor:pointer">💰 Registrar abono</button>';
    html += '<div id="ab-err" style="font-size:11px;color:var(--d);min-height:14px;margin-top:4px"></div>';
    html += '</div>';
  }

  // REGISTRO DE PAGO EFECTIVO (pago completo en un solo pago)
  html += sec('Marcar como pagado en su totalidad');
  ...
  html += '<div style="font-size:10px;color:var(--t2);margin-bottom:10px">Usa esto solo si el pago se hizo completo, de una sola vez. Si fue parcial, usa "Registrar abono" arriba.</div>';
  html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:8px">';
...
async function guardarAbonoNomina(id, empresa) {
  var btn = document.querySelector('#m-b button[onclick^="guardarAbonoNomina"]');
  var errEl = $('ab-err');
  var parseNum = function(v){
    if(v==null||v==='') return 0;
    var s = String(v).trim();
  ...
  var valor = parseNum($('ab-valor')?.value);
  var fecha = $('ab-fecha')?.value || new Date().toISOString().split('T')[0];
  var cuenta = ($('ab-cuenta')?.value||'').trim()||null;

  if (!valor || valor <= 0) { if(errEl) errEl.textContent = 'Ingresa un monto válido'; return; }
  if (btn) { btn.textContent = 'Registrando...'; btn.disabled = true; }

  var {data: p} = await db.from('nomina_pagos').select('*').eq('id', id).single();
  if (!p) { if(errEl) errEl.textContent = 'Pago no encontrado'; if(btn){btn.textContent='💰 Registrar abono';btn.disabled=false;} return; }

  // Convertir el abono a USD usando la moneda/TRM del pago original
  var moneda = (p.moneda||'COP').toUpperCase();
  var trm = parseFloat(p.trm)||0;
  var valorUsd = null;
  if (moneda === 'USD') valorUsd = valor;
  else if (trm > 0) valorUsd = moneda==='COP' ? Math.round((valor/trm)*100)/100 : Math.round((valor*trm)*100)/100;

  var {error: errAbono} = await db.from('nomina_abonos').insert({
    nomina_pago_id: id, empresa, valor, valor_usd: valorUsd, moneda, trm: trm||null,
    fecha, cuenta_pago: cuenta
  });
  if (errAbono) { if(errEl) errEl.textContent = 'Error: '+errAbono.message; if(btn){btn.textContent='💰 Registrar abono';btn.disabled=false;} return; }

  // Recalcular total abonado y decidir el nuevo estado
  var {data: todosAbonos} = await db.from('nomina_abonos').select('valor').eq('nomina_pago_id', id);
  var totalAbonado = (todosAbonos||[]).reduce(function(s,a){return s+(a.valor||0);},0);
  var netoObjetivo = p.valor_neto || 0;

  var payload;
  if (totalAbonado >= netoObjetivo && netoObjetivo > 0) {
    var prog = p.fecha_pago ? new Date(p.fecha_pago+'T12:00:00') : null;
    var real = new Date(fecha+'T12:00:00');
    var diasMora = prog ? Math.round((real-prog)/86400000) : null;
    payload = { estado: 'Pagado', fecha_pago_real: fecha, cuenta_pago: cuenta || p.cuenta_pago, dias_mora: diasMora, updated_at: new Date().toISOString() };
  } else {
    payload = { estado: 'Abono', updated_at: new Date().toISOString() };
  }
  await db.from('nomina_pagos').update(payload).eq('id', id);

  toast(payload.estado==='Pagado' ? 'Abono registrado — pago completado ✓' : 'Abono registrado — saldo pendiente '+fm(Math.max(netoObjetivo-totalAbonado,0)), 'ok');
  closeM();
  await loadNomina(empresa);
}
...
// ══════════════════════════════════════════════════════════════
function resetFiltrosReporte(suf){
  var empresa = suf==='ty'?'tycoon':'diaz';
  ['desde','hasta','tipo','estado','empleado'].forEach(function(campo){
    var el=$('rnom-'+suf+'-'+campo);
    if(el) el.value='';
  });
  loadReporteNomina(empresa);
}
...
async function loadReporteNomina(empresa) {
  ...
  // Construir query — pagos que vencen/corresponden a este período
  ...
  var {data: rowsPeriodo, error} = await q;
  if(error){ toast('Error cargando reporte: '+error.message,'d'); return; }
  rowsPeriodo = rowsPeriodo || [];

  // Cartera arrastrada: abonos/pendientes de períodos ANTERIORES a "desde" que
  // aún no se han saldado — así el saldo con mora aparece en el reporte actual
  var rowsArrastrados = [];
  if (desde) {
    var qArr = db.from('nomina_pagos').select('*').eq('empresa', empresa).in('estado', ['Abono','Pendiente']).lt('periodo_fin', desde);
    if(tipo) qArr = qArr.eq('tipo', tipo);
    var {data: arr} = await qArr;
    rowsArrastrados = arr || [];
  }
  var idsPeriodo = new Set(rowsPeriodo.map(function(r){return r.id;}));
  rowsArrastrados = rowsArrastrados.filter(function(r){return !idsPeriodo.has(r.id);});
  rowsArrastrados.forEach(function(r){ r._arrastrado = true; });

  var rows = rowsPeriodo.concat(rowsArrastrados);
  await enriquecerConAbonos(rows);
  ...
  // Saldo pendiente total (incluye Abono/Pendiente, sean de este período o arrastrados)
  var rowsAbiertos = rows.filter(function(r){return r.estado==='Abono'||r.estado==='Pendiente';});
  var saldoPendienteTotal = rowsAbiertos.reduce(function(a,r){return a+(r._saldo!=null?r._saldo:(r.valor_neto||0));},0);
  var numArrastrados = rows.filter(function(r){return r._arrastrado;}).length;
  ...
  kpis = [
    ...
    ['Saldo pendiente',fm(saldoPendienteTotal),numArrastrados>0?(numArrastrados+' arrastrado(s) de períodos anteriores'):'abonos y pendientes de este período',saldoPendienteTotal>0?'#b8590a':'var(--t3)']
  ].map(function(k){
    return '<div style="background:var(--sf);border:1px solid var(--br);border-radius:12px;padding:14px 16px;border-top:3px solid '+k[3]+'">'
  ...
  // Tabla
  var estCol = {Pagado:'#00554B',Pendiente:'#d4870a',Procesando:'#5B8DB8',Abono:'#b8590a'};
  ...
  var rows_html = rows.length ? rows.map(function(r) {
    ...
    var saldoCell = (r.estado==='Abono'||r.estado==='Pendiente')
      ? fm(r._saldo!=null?r._saldo:(r.valor_neto||0))+(r._totalAbonado>0?'<br><span style="font-size:8px;color:var(--t3)">abonado '+fm(r._totalAbonado)+'</span>':'')
      : '—';
    var arrastradoTag = r._arrastrado ? '<br><span style="font-size:8px;padding:1px 5px;border-radius:6px;background:rgba(184,89,10,0.12);color:#b8590a;font-weight:700">QUINCENA ANTERIOR</span>' : '';
    ...
    return '<tr>'
      ...
      +'<td style="font-size:10px;color:var(--t2)">'+(r.periodo_inicio?fd(r.periodo_inicio):'—')+(r.periodo_fin?'<br>→ '+fd(r.periodo_fin):'')+arrastradoTag+'</td>'
      ...
      +'<td style="text-align:right;font-family:monospace;font-size:12px;font-weight:700;color:'+(r.estado==='Abono'?'#c0392b':'var(--t2)')+'">'+saldoCell+'</td>'
      ...
      +'</tr>';
  }).join('') : '<tr><td colspan="13" class="mod-empty">Sin pagos en este período</td></tr>';
  ...
  var totales = '<tfoot><tr style="border-top:2px solid var(--br)">'
    ...
    +'<td style="text-align:right;font-family:monospace;font-size:12px;font-weight:700;color:#b8590a">'+fm(saldoPendienteTotal)+'</td>'
...
<div class="body">
  ...
  <table>
    <thead><tr>
      ...
      <th class="r">Bruto</th><th class="r">Descuento</th><th class="r">Neto</th><th class="r">A pagar ahora</th>
      <th>Observación descuento</th><th>Límite pago</th><th>Estado</th>
    </tr></thead>
...
<script>
  ...
  var totalB=0,totalD=0,totalN=0,totalPagarAhora=0,totalEmp=new Set();
  var tipCol={Quincenal:'#1565c0',Mensual:'#6a1b9a','Por horas':'#1b5e20',Comision:'#e65100',Otro:'#37474f'};
  var estCls={Pagado:'badge-ok',Pendiente:'badge-p',Procesando:'badge-pr',Abono:'badge-p'};
  var tbody = document.getElementById('print-tbody');
  rows.forEach(function(r){
    totalB+=(r.valor_bruto||0); totalD+=(r.deducciones||0); totalN+=(r.valor_neto||0);
    // Monto a pagar ahora: si es Abono/Pendiente, el saldo real (no el neto original completo)
    var montoAhora = (r.estado==='Abono'||r.estado==='Pendiente') ? (r._saldo!=null?r._saldo:(r.valor_neto||0)) : 0;
    totalPagarAhora += montoAhora;
    ...
    var dm = (r._diasMora!=null) ? r._diasMora : r.dias_mora;
    var mora='';
    if(dm!=null){if(dm>0)mora='<br><span style="color:#8b0000;font-size:7pt">+'+dm+'d mora</span>';else if(dm<0)mora='<br><span style="color:#155724;font-size:7pt">-'+Math.abs(dm)+'d</span>';}
    ...
    var arrastradoTag = r._arrastrado ? ' <span style="font-size:6pt;color:#b8590a;font-weight:700">(QUINCENA ANTERIOR)</span>' : '';
    var per=(r.periodo_inicio?new Date(r.periodo_inicio+'T12:00:00').toLocaleDateString('es-CO',{day:'2-digit',month:'short'}):'—')+(r.periodo_fin?'<br>'+new Date(r.periodo_fin+'T12:00:00').toLocaleDateString('es-CO',{day:'2-digit',month:'short',year:'numeric'}):'')+arrastradoTag;
    var fmtCO=function(n){return '$'+(n||0).toLocaleString('es-CO');};
    var montoAhoraCell = (r.estado==='Abono'||r.estado==='Pendiente') ? fmtCO(montoAhora) : '—';
    ...
    tr.innerHTML='<td><strong>'+(r.empleado_nombre||'—')+'</strong><br><span style="font-size:7pt;color:#5c6c7c">'+banco+'</span></td>'
      ...
      +'<td class="r" style="color:#b8590a">'+montoAhoraCell+'</td>'
  ...
  totRow.innerHTML='<td colspan="4">Totales ('+totalEmp.size+' empleados)</td>'
    ...
    +'<td class="r" style="color:#b8590a">'+fmt(totalPagarAhora)+'</td>'
    +'<td colspan="3"></td>';
  tbody.appendChild(totRow);
  // KPIs
  var kpiData=[['Empleados',totalEmp.size,'en este período'],['Total bruto',fmt(totalB),'devengado'],['Descuentos',fmt(totalD),'deducciones'],['Neto a pagar',fmt(totalN),'valor original'],['Saldo pendiente',fmt(totalPagarAhora),'incluye arrastrados']];
...
<script>
  ...
  // Solo se transfiere lo que aún se debe: se excluye lo ya Pagado por completo
  var rowsATransferir = rows.filter(function(r){ return r.estado !== 'Pagado'; });

  // Agrupar por empleado para poder mostrar subtotal cuando tiene más de un pago
  var porEmpleado = {};
  var ordenEmpleados = [];
  rowsATransferir.forEach(function(r){
    var key = r.empleado_nombre || '—';
    if(!porEmpleado[key]) { porEmpleado[key] = []; ordenEmpleados.push(key); }
    porEmpleado[key].push(r);
  });

  var fila = 0;
  ordenEmpleados.forEach(function(nombreEmp){
    var filasEmp = porEmpleado[nombreEmp];
    var subtotalNeto = 0, subtotalNetoUSD = 0, subtotalUSDValido = true;

    filasEmp.forEach(function(r){
      fila++;
      ...
      var moneda = (r.moneda||'COP').toUpperCase();
      var trm = parseFloat(r.trm)||0;
      // Si ya hubo abono, se transfiere solo el saldo — no el valor original completo
      var neto = (r.estado==='Abono' && r._saldo!=null) ? r._saldo : (r.valor_neto||0);
      var netoFmt = (moneda==='USD')
        ? '$'+neto.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})
        : '$'+neto.toLocaleString('es-CO');
      // Días de mora en vivo — visible cuando el pago viene vencido o arrastrado de un período anterior
      var dm = r._diasMora!=null ? r._diasMora : r.dias_mora;
      var moraTag = (dm!=null && dm>0) ? '<br><span style="font-size:6pt;color:#8b0000;font-weight:700">+'+dm+'d mora</span>' : '';
      var arrastradoTag = r._arrastrado ? ' · quincena anterior' : '';
      var monLabel = '<span class="neto-mon">'+moneda+arrastradoTag+'</span>'+moraTag;
      var proporcion = (r.valor_neto_usd!=null && r.valor_neto>0) ? (neto/r.valor_neto) : 1;
      var netoUSD = (r.valor_neto_usd!=null) ? r.valor_neto_usd*proporcion
                  : (moneda==='USD') ? neto
                  : (moneda==='COP' && trm>0) ? neto/trm
                  : (moneda==='EUR' && trm>0) ? neto*trm
                  : null;
      ...
      totalNeto += neto;
      subtotalNeto += neto;
      if(netoUSD!=null) { totalNetoUSD += netoUSD; subtotalNetoUSD += netoUSD; } else { subtotalUSDValido = false; }
      ...
      if(r.estado==='Abono') {
        var moraTxt = (dm!=null && dm>0) ? ' · '+dm+' días de mora' : '';
        novedades.push({nombre:r.empleado_nombre, nota:'Abono previo de $'+(r._totalAbonado||0).toLocaleString('es-CO')+' — este pago es el saldo restante'+moraTxt, dedu:0});
      }

      var tr = document.createElement('tr');
      tr.innerHTML =
        '<td style="font-family:Courier New,monospace;color:#6b7280;font-size:6.5pt">'+fila.toString().padStart(2,'0')+'</td>'
    ...
    // Subtotal por empleado — solo si tiene más de un pago pendiente en este reporte
    if (filasEmp.length > 1) {
      var subRow = document.createElement('tr');
      subRow.style.background = '#f6f8fb';
      subRow.innerHTML = '<td></td>'
        +'<td colspan="2" style="font-size:6.5pt;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px">Total a pagar · '+nombreEmp+'</td>'
        +'<td class="r" style="font-weight:800;color:#0d1f35">$'+subtotalNeto.toLocaleString('es-CO')+'</td>'
        +'<td class="r" style="font-weight:800;color:#374151">'+(subtotalUSDValido?'$'+subtotalNetoUSD.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}):'—')+'</td>';
